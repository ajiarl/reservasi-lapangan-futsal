...
// AJAX: Get jadwal by lapangan & hari
if (isset($_GET['ajax']) && $_GET['ajax'] == 'get_jadwal') {
    $lapangan_id = intval($_GET['lapangan_id'] ?? 0);
    $hari = $_GET['hari'] ?? '';
    
    if ($lapangan_id && $hari) {
        $jadwal = fetchAll("SELECT jadwal_id, jam_mulai_slot, jam_selesai_slot, harga_perjam_slot 
            FROM jadwallapangan WHERE Lapangan_lapangan_id=? AND hari=? ORDER BY jam_mulai_slot", 
            [$lapangan_id, $hari]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        foreach ($jadwal_ids as $i => $jadwal_id) {
            $jadwal = fetchOne("SELECT harga_perjam_slot FROM jadwallapangan WHERE jadwal_id=?", [intval($jadwal_id)]);
        ...
        $conn->rollback();
        header('Location: index.php?error=Gagal menambahkan booking');
        exit;
    }
}

$users = fetchAll("SELECT user_id, nama_pelanggan, email FROM users ORDER BY nama_pelanggan");
$lapangan = fetchAll("SELECT lapangan_id, nama_lapangan FROM lapangan WHERE is_active=1 ORDER BY nama_lapangan");
...
<div class="card">
    ...
    <form method="POST" id="form" style="max-width:800px;">
        <div class="form-group">
            <label>Pilih Pelanggan *</label>
            <select name="user_id" class="form-control" required>
                <option value="">-- Pilih Pelanggan --</option>
                <?php foreach ($users as $u): ?>
                    <option value="<?= $u['user_id'] ?>"><?= htmlspecialchars($u['nama_pelanggan']) ?> (<?= $u['email'] ?>)</option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <input type="date" name="tgl_booking" class="form-control" min="<?= date('Y-m-d') ?>" required>
        </div>
        
        <div class="form-group">
            <label>Slot Waktu</label>
            <button type="button" class="btn btn-sm" onclick="addSlot()">+ Tambah Slot</button>
        </div>
...
function addSlot() {
    ...
    div.innerHTML = `
        ...
        <div style="margin-bottom:10px;">
            ...
            <select name="jadwal_id[]" id="jadwal_${counter}" class="form-control" onchange="updateJam(${counter})" required>
...
function loadJadwal(id) {
    const lapangan = document.querySelector(`select[name="lapangan_${id}"]`).value;
    const hari = document.querySelector(`select[name="hari_${id}"]`).value;
    const select = document.getElementById('jadwal_' + id);
    
    if (!lapangan || !hari) return;
    
    fetch(`?ajax=get_jadwal&lapangan_id=${lapangan}&hari=${hari}`)
        .then(r => r.json())
        .then(data => {
            select.innerHTML = '<option value="">-- Pilih Jadwal --</option>';
            data.forEach(j => {
                select.innerHTML += `<option value="${j.jadwal_id}" data-mulai="${j.jam_mulai_slot}" data-selesai="${j.jam_selesai_slot}">${j.jam_mulai_slot} - ${j.jam_selesai_slot} (Rp ${parseInt(j.harga_perjam_slot).toLocaleString('id-ID')})</option>`;
            });
        });
}
